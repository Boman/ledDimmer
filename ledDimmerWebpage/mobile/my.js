...
LED_NUM_EXTRA_PROGRAMS = 3;
programNames = {
	'AUTO' : 1 + LED_NUM_EXTRA_PROGRAMS,
	'FLASH' : 2 + LED_NUM_EXTRA_PROGRAMS,
	'JUMP3' : 3 + LED_NUM_EXTRA_PROGRAMS,
	'JUMP7' : 4 + LED_NUM_EXTRA_PROGRAMS,
	'FADE3' : 5 + LED_NUM_EXTRA_PROGRAMS,
	'FADE7' : 6 + LED_NUM_EXTRA_PROGRAMS
};
...
function rpcCall(adress, lightType, value) {
	var send = false;
	...
	if (send) {
		$('#log').append("(" + adress + ", " + lightType + ", " + value + ")");
	}
}
...
$(function() {
	...
	function(session) {
		sess.subscribe("http://unserhaus.lan/event#light_set_event", function(topic, event) {
			alert(event);
		});
	},
